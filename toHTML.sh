...
convertToHTML(){
  l=`grep -nrH h1 "${WORKDIR}Plugin-API.html" | head -1 | cut -f2 -d':'`
  ...
  l=`grep -nrH h1 "${WORKDIR}Waylay-REST-API-documentation.html" | head -1 | cut -f2 -d':'`
  ...
  l=`grep -nrH h1 "${WORKDIR}Submitting-and-fetching-data.html" | head -1 | cut -f2 -d':'`
  ...
  l=`grep -nrH h1 "${WORKDIR}Waylay-starters-guide.html" | head -1 | cut -f2 -d':'`
  ...
  l=`grep -nrH h1 "${WORKDIR}Architecture.html" | head -1 | cut -f2 -d':'`
  ...
  l=`grep -nrH h1 "${WORKDIR}Docker-image-guide.html" | head -1 | cut -f2 -d':'`
  ...
  l=`grep -nrH h1 "${WORKDIR}Tasks-and-Templates.html" | head -1 | cut -f2 -d':'`
